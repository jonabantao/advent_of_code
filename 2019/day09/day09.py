class Solution():
    def solve_a(self, filename):
        with open(filename, "r") as f:
            ...
            input_list.extend([0] * max(input_list))

            return next(self.run_intcode_computer(input_list, [1]))
    ...
    def solve_b(self, filename):
        with open(filename, "r") as f:
            ...
            return next(self.run_intcode_computer(input_list, [2]))
    ...
    def run_intcode_computer(self, intcode, values):
        ...
        while True:
            instruction_count = 0
            op_code, mode1, mode2, mode3 = self.translate_modes(intcode[curr_ptr])

            if op_code == 99:
                return

            if op_code in [1, 2, 5, 6, 7, 8, 9]:
                param1 = self.translate_value(curr_ptr + 1, mode1, intcode, curr_base)
                param2 = self.translate_value(curr_ptr + 2, mode2, intcode, curr_base)
                param3 = self.translate_value(curr_ptr + 3, mode3, intcode, curr_base)

                if op_code == 1:
                    intcode[param3] = self.op_code1(intcode[param1], intcode[param2])
                elif op_code == 2:
                    intcode[param3] = self.op_code2(intcode[param1], intcode[param2])
                elif op_code == 5:
                    new_ptr_value = self.op_code5(intcode[param1], intcode[param2])

                    curr_ptr = new_ptr_value if new_ptr_value != -1 else curr_ptr + 3
                elif op_code == 6:
                    new_ptr_value = self.op_code6(intcode[param1], intcode[param2])

                    curr_ptr = new_ptr_value if new_ptr_value != -1 else curr_ptr + 3
                elif op_code == 7:
                    self.op_code7(intcode[param1], intcode[param2], param3, intcode)
                elif op_code == 8:
                    self.op_code8(intcode[param1], intcode[param2], param3, intcode)
                elif op_code == 9:
                ...
                raise Exception("Your computer asplode at op_code {}.".format(op_code))
